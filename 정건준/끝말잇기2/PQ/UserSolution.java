...
public class UserSolution {
    ...
    static PriorityQueue<String>[] wordPQ; //wordPQ[c] := c 로 시작 하는 사용 가능한 단어 집합
    ...
    void init(int N, int M, char mWords[][]) {
         ...
         select = new HashSet<>();
         wordPQ = new PriorityQueue[26];

         for(int i=0; i<26; i++) wordPQ[i] = new PriorityQueue<>();
         ...
         for(int i=0; i<M; i++) {
             ...
             wordPQ[sb.charAt(0) - 'a'].offer(sb.toString());
         }
    }
    ...
    int playRound(int mID, char mCh) {
        ...
        while(true) {
            //단어를 뽑음
            String word = wordPQ[curCh - 'a'].poll();
            select.add(word);
            ...
            //뒤집힌 단어가 아직 뽑히지 않았으면 추가될 가능성이 있음
        ...
        for(int i=0; i<nextPushList.size(); i++) {
            ...
            int pqIdx = word.charAt(0) - 'a';
            //(TreeSet과 다른 점), PQ에 해당 단어가 있으면 넣지 않음
            //why?, treeSet은 중복 제거, PQ는 그렇지 않음
            if(!select.contains(word) && !wordPQ[pqIdx].contains(word))
                wordPQ[pqIdx].add(word);
        }
